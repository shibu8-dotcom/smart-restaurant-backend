...
def generate_pdf_bill(order, bill_obj, output_dir='static/bills'):
    ...
    c.drawString(40, height - 85, f"Table: {order.table_no or '-'}")
    c.drawString(40, height - 100, f"Date: {order.created_at.strftime('%Y-%m-%d %H:%M:%S')}")

    c.setFont("Helvetica-Bold", 11)
    c.drawString(40, height - 130, "Items")
    c.drawString(350, height - 130, "Qty")
